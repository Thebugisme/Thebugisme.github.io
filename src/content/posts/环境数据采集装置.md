...
draft: false
lang: ''
slug: huan-jing-shu-ju-cai-ji-zhuang-zhi
---
## 主要材料

stm32c6t6开发板，0.96寸oled显示屏、DHT11温湿度模块、光敏电阻模块

## 功能

读取传感器的数据并实时显示在oled屏幕上

## 成果展示

![](https://bee-oss-ab.imglnk.cn/u/20260904/13a1a4d1c5cacfb11bfbacf4d1c0303d.jpg)
...
char DHT11_Rec_Byte(void)
{
    ...
    for(i = 0; i < 8; i++)
    {
        ...
        delay_us(30);                    // 延迟30us判断数据0或1
        
        data <<= 1;                      // 左移
        
        if(Read_Data == 1)               // 如果还是高电平就是数据1
        {
            data |= 1;
        }
...
void DHT11_REC_Data(void)
{
    ...
    unsigned char RH = 0, RL = 0, TH = 0, TL = 0;
    unsigned char CHECK;
    ...
    if(Read_Data == 0)                   // 判断DHT11是否响应
    {
        ...
        dht11_low;                       // 最后一bit传送完毕后，拉低总线50us
        delay_us(55);
        dht11_high;                      // 总线由上拉电阻拉高进入空闲状态
        
        if(R_H + R_L + T_H + T_L == CHECK)  // 校验数据是否正确
        {
            RH = R_H;
            RL = R_L;
            TH = T_H;
...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
char temp_str[32] = {0};  // 温度字符串
char hum_str[32] = {0};   // 湿度字符串
/* USER CODE END 0 */
...
int main(void)
{
  ...
  while (1)
  {
    ...
    HAL_Delay(1000);
    DHT11_REC_Data();
    sprintf(temp_str, "温度: %.1d C", rec_data[2]+rec_data[3]/10);
    sprintf(hum_str, "湿度: %.1d %%", rec_data[0]+rec_data[1]/10);
    OLED_NewFrame(); // 新建一个空白缓冲区
    OLED_PrintString(0, 0, temp_str, &font16x16, OLED_COLOR_NORMAL);
    OLED_PrintString(0, 22, hum_str, &font16x16, OLED_COLOR_NORMAL);
